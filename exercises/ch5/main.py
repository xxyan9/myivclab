...
# min_q_scale = [0.1, 0.2, 0.3, 0.5, 0.6, 0.8, 1.0, 1.5, 2]
# max_q_scale = [0.3, 0.8, 1.2, 2, 3, 4, 5, 8, 10]

# min_q_scale = [0.4, 1.0, 1.5]
# max_q_scale = [0.8, 2, 3]

min_q_scale = [0.4]
max_q_scale = [0.5]

for min_q, max_q in zip(min_q_scale, max_q_scale):
    intra_codec = IntraCodec(min_q=min_q, max_q=max_q, use_adaptive_quant=True)
    intra_codec.train_huffman_from_image(lena_small)
    ...
    for frame_num, image in enumerate(images):
        ...
        bpps.append(bpp)
        psnrs.append(psnr)
        print(f"PSNR: {psnr:.2f} dB, bpp: {bpp:.2f}")
    all_bpps.append(np.mean(bpps))
    all_psnrs.append(np.mean(psnrs))
    print(f"min Q Scale: {min_q}, max Q Scale: {max_q}, PSNR: {psnr:.2f} dB, bpp: {bpp:.2f}")
...
# 5. Half pel motion compensation
# 6. Quarter pel motion compensation
