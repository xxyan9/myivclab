...
# 3. Chapter 5-1: In-loop Deblocking Filter
images = []
for i in range(20, 40 + 1):
    images.append(imread(f'../data/foreman20_40_RGB/foreman00{i}.bmp'))
...
ch5_1_bpps = np.array(all_bpps)
ch5_1_psnrs = np.array(all_psnrs)

np.save('../data/ch5_1_bpps.npy', ch5_1_bpps)
np.save('../data/ch5_1_psnrs.npy', ch5_1_psnrs)
...
ch5_1_bpps = np.load('../data/ch5_1_bpps.npy')
ch5_1_psnrs = np.load('../data/ch5_1_psnrs.npy')
...
plt.plot(ch5_1_bpps, ch5_1_psnrs, linestyle='--', marker='x', color='purple', label='Optimization 1')
plt.legend()
plt.show()