...
class QuarterPelMotionCompensator:
    ...
    def block_matching_quarter_pel(self, current_block, reference_image, loc_x, loc_y):
        """
        三层分层搜索：整数 -> 半像素 -> 四分之一像素 (基于MATLAB版本的思路)
        """
        ...
        # Step 2: 半像素搜索 (基于最佳整数位置，搜索4个半像素位置)
        # 遵循MATLAB的4个半像素搜索模式
        half_pel_patterns = [
        ...
        # Step 3: 四分之一像素搜索 (基于最佳半像素位置，搜索4个四分之一像素位置)
        # 遵循MATLAB的4个四分之一像素搜索模式
...
class VideoCodec:
    ...
    def __init__(self,
        ...
        self.search_range = search_range
        self.use_quarter_pel = use_quarter_pel  # 是否使用四分之一像素运动估计
    ...
    def encode_decode(self, frame, frame_num=0):
        if frame_num == 0:
            ...
            # Motion vector computation (支持四分之一像素)
